...
_DEFAULTS: dict = {
    ...
    "settings": {
        ...
        "auto_accept":        False,
        "auto_accept_delay":  0,
        # Default OFF to prevent spam. Admin can enable from Settings.
...
async def on_join_request(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Already recorded → send the same request/host messages again ───────────────
    # This also handles users who left, received a leave DM, and later request again.
    ...
    bot_data["pending_requests"][uid_str] = {
        ...
        "rejoin_request": uid_str in bot_data.get("left_members", []),
    }
    if uid_str in bot_data.get("declined_users", []):
        bot_data["declined_users"].remove(uid_str)
    ...
    rejoin_note = f"\n{E_REFRESH} <b>Rejoin:</b> Yes — user had left before" if uid_str in bot_data.get("left_members", []) else ""
    admin_text = (
...
async def on_chat_member(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if old_status in LEFT_STATUSES and new_status in ACTIVE_STATUSES:
        ...
        if bot_data["settings"].get("join_notifications", False):
            first_name = user.first_name or "there"
    ...
    # ── LEFT / KICKED ────────────────────────────────────
        ...
        if bot_data["settings"].get("leave_notifications", False):
            first_name = user.first_name or "there"
...
async def _show_settings(q, ctx):
    ...
    text = (
        ...
        f"{E_GREEN} Join Notifications : {yn(join_notify)}\n"
        f"{E_RED} Leave Notifications : {yn(leave_notify)}\n"
    ...
    kb = InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton(
                f"{'🔴 Disable' if join_notify else '🟢 Enable'} Join DM",
                callback_data="toggle_join_notifications",
            ),
            InlineKeyboardButton(
                f"{'🔴 Disable' if leave_notify else '🟢 Enable'} Leave DM",
